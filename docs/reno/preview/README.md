# ReNo v1.2 — standalone test preview (dummy)

A **self-contained, single-file** run of the whole new **ReNo v1.2 five-axis Likert** test, built for
review/approval **before** anything on prod changes. Open
[`reno-v1.2-preview.html`](reno-v1.2-preview.html) directly in a browser — no server, no build, no
...
- Uses the **already-created** v1.2 content, not new questions:
...
## Scoring (ReNo v1.2 — QuestionBank §05)
...
```
python3 docs/reno/preview/build.py    # _preview.template.html + JSON data -> reno-v1.2-preview.html
```
